...
Bundles require a bundle-root `index.html`. Single-file HTML uploads are normalized to `{ownerUserId}/{bundleId}/index.html`; zip uploads may contain `index.html` at the zip root or wrap the whole bundle in one top-level directory. When a zip has only one top-level directory, validation treats that directory as the bundle root and stores its contents under canonical relative paths such as `{ownerUserId}/{bundleId}/index.html`. Upload validation builds an internal manifest of normalized paths, sizes, and content types, writes each object with S3 `ContentType`, ignores common archive noise such as `__MACOSX/**`, `.DS_Store`, and `Thumbs.db`, and rejects root-relative references that would escape the token namespace. Zip entry paths still cannot contain `..` segments, but page references and served bundle paths may use `../` when normalization stays inside the bundle root. Large binary assets are classified by path/content type and are not scanned as UTF-8. The V1 read path keeps `/t/{token}` as the user-facing share URL and serves `/t/{token}/...` through the API/Lambda so relative HTML, CSS, JavaScript modules, images, and other references resolve inside the same token namespace.
...
- Root-relative links are rejected up front rather than served broken later. Up-directory links such as `../index.html` are allowed for nested pages as long as request-path normalization stays inside the uploaded bundle.
